...
sub _write_file {
    ...
    truncate FH, 0;
    my $bytes = 0;
    if (! MT->run_callbacks( 'MT::FileMgr::Local::pre_put_data',
        $fmgr, \$from, \$to, $type, $bytes ) ) {
        return undef;
    }
    if ( $fmgr->is_handle($from) ) {
    ...
    umask($old);
    if (! $cfg->NoTempFiles ) {
        if ( !$fmgr->rename( $to, $original, 'no_cb' ) ) {
            return $fmgr->error(
            MT->translate(
    ...
    MT->run_callbacks( 'MT::FileMgr::Local::put_data', $fmgr, $from, $to, $type, $bytes );
    $bytes;
}
...
sub mkpath {
    ...
    umask($old);
    MT->run_callbacks( 'MT::FileMgr::Local::mkpath', $fmgr, $path );
    1;
}
...
sub rename {
    ...
    if ( $fmgr->exists($to) ) {
        $fmgr->delete($to,'no_cb') or return;
    }
    # rename $from, $to
    require File::Copy;
    ...
    if (! $no_cb ) {
        MT->run_callbacks( 'MT::FileMgr::Local::rename', $fmgr, $from, $to );
    }
    1;
}
...
sub delete {
    ...
    if (! $no_cb ) {
        MT->run_callbacks( 'MT::FileMgr::Local::delete', $fmgr, $file );
    }
    1;
}
